...
export default function CalculatorTemplate({
  ...
  const handleCopy = () => {
    ...
    const text = Object.entries(result)
      ...
      .map(
        ([k, v]) =>
          `${friendlyLabel(k)}: ${typeof v === "number" ? formatIndian(v) : v}`,
      )
      .join("\n");
          ...
          <div className="grid md:grid-cols-2 gap-6 items-start">
              ...
              <div className="calc-panel-inputs">
                ...
                <div className="calc-inputs">
                  {inputs.map((input) => (
                    <div
                      key={input.name}
                      className={`calc-field ${errors[input.name] ? "has-error" : values[input.name] !== "" ? "has-value" : ""}`}
                    >
                      ...
                      {errors[input.name] && (
                        <span className="calc-field-error">
                          ⚠ {errors[input.name]}
                        </span>
                      )}
                    </div>
                ...
                <div className="calc-actions">
                  <button
                    ...
                    onClick={() => {
                      setTouched(
                        Object.fromEntries(inputs.map((i) => [i.name, true])),
                      );
                      calculate(values);
                    }}
                  >
            ...
            <div>
              ...
              <div className="calc-panel-result">
                ...
                {hasResult && (
                  <div
                    className={`calc-results ${animate ? "calc-results-in" : ""}`}
                  >
                    {Object.entries(formattedResult)
                      ...
                      .map(([k, v], i) => {
                        ...
                        return (
                          <div
                            key={k}
                            className={`calc-result-row ${isFirst ? "calc-result-primary" : ""} ${isFormula ? "calc-result-formula" : ""}`}
                          >
